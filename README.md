...
2024-05-19
-----------
Alright! I've now fixed some minor (and some less minor) problems in the frontend. First, I started by moving all the HTTP requests into one file. I knew I should've done this from the start, but I finally got around to doing it. And the result is definetely worth it, I think. I'm using Axios for the actual HTTP requests, and it all went well, all things considered. One thing that sort of caught me off-guard is that Axios throws an exception when the response isn't in the 2xx-range. I had not accounted for this, and so my error handling had to be refactored. In the end, this meant an improvement, though. So, no complaints there. Also, I fixed up the error handling in `TakeQuiz.jsx` as well. It isn't perfect, but at least it's existent. If something goes wrong while getting a quesiton or checking an answer, that error message is displayed. It's displayed as a question, but you can't answer it. I think it's a little more clear than it sounds like, but what do I know? Also, because of how I handled answer checking in the backend, when a request to check the answer fails, you get another shot at it. So, if the first request fails (for some reason), you can click `Get Question` to get it again. So that's a nice... feature.

2024-05-20
-----------
I noticed a very minor 'problem' in a utils file. To call it a problem is proably a stretch, though. It was that I was exporting a function that was only used within the utils file in question. So, I removed the `export` keyword, verified that nothing broke, and done.

Hmkay. In the backend, I've now added a unique constraint to the `Question` property of each question. I already had a way to check for duplicate questions, but this is more efficient (I think?). So now, instead of searching through the database for potential duplicates, instead I just let the insertion happen. But because of the constraint, an exception is thrown if the question is a duplicate. I think the exception is handled properly, but I can't gurantee anything. But hey, at least it's working. It might be worth to note that this came to my attention through a warning in the debugger output of Visual Studio. EF Core was warning me that I was fetching items without an `OrderBy`, which could bring unpredictable results (the order of the items isn't always preserved, it seems). This wasn't exactly relevant in my case, but it led to a change. So that's good, I think.

2024-05-21
-----------
Not sure how I always seem to forget about code cleanups. So I ran one today, since I forgot to do so yesterday, and it adjusted two small things in `QuizService.cs`. Nothing massive. Also, I wrote a PowerShell script (two, technically) to launch the project faster. So, I simply open a PowerShell instance, run `StartQuiz` and boom. It changes directory to the correct one, opens two new windows, changes their active directories to the backend and frontend respectively, and boots them up. I'd barely ever written PowerShell scripts (or any other terminal scripts, for that matter) before, but I definitely think I'll be doing more so, from now on.
